class TaskManager {
    ...
    updateSummary() {
        ...
        const meet = this.tasks.filter(task => task.status === '-').length;
        ...
        document.getElementById('-').textContent = done;
    ...
    generateEmailBody() {
        ...
        const done = this.tasks.filter(task => task.status === '-').length;
        const completed = this.tasks.filter(task => task.status === 'Completed').length;
        const inProgress = this.tasks.filter(task => task.status === 'In Progress').length;
        const totalHours = this.tasks.reduce((sum, task) => sum + task.estimatedHours, 0);
...
Task Details:
------------------------------------------------------------
| Title              | Priority | Status       | Hours | Date       |
------------------------------------------------------------\n`;
...
[your name]
    ...
    exportToCSV() {
        ...
        const headers = ['Date', 'Task Title', 'Priority', 'Status', 'Estimated Hours', 'Created At'];
        const csvContent = [
            headers.join(','),
            ...this.tasks.map(task => [
                `"${task.date}"`,
